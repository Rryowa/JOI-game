...
>> = {
  ...
  [GameEvent.doublePace]: { base: 0.005, scale: 0.03, category: 'modifier' },
...
>> = {
  [GameEvent.randomGrip]: { base: 0.01, scale: 0.01 },
};
...
export const rollEventDice = (
  ...
  const candidates = events
    .filter(e => config[e as keyof typeof config])
    .map(e => {
...
export const GameEvents = () => {
  ...
  useLooping(
    async () => {
      const event = rollEventDice(data, MAIN_EVENT_CONFIG, lastMainEventTimestamp, EVENT_COOLDOWN);
      if (event) {
  ...
  useLooping(
    async () => {
      const event = rollEventDice(data, VISUAL_EVENT_CONFIG, lastVisualEventTimestamp, 3); // Shorter 3s cooldown for visuals
      if (event) {
